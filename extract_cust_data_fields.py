import sys
import time
import uopy

savedListName = "SAVED_CUST"

def make_connection(user, password):
    config = {
             'user': user,
             'password': password,
             'service': 'uvcs',
             'host':    'localhost',
             'account': 'HS.SALES',
             #'encoding': 'GB18030',
         }
    try:
        status = 0
        thissession = uopy.connect(**config)
    except uopy.UOError as e:
        print(str(e.code))
        status = e.code
        thissession = "not connected"
    return status, thissession

def select_items():
    # this is a UniVerse SELECT command -- no semi-colon at end!
    cmd1 = uopy.Command("SELECT CUSTOMER @ID WITH BUY_DATE EQ '01/08/1991'")
    cmd1.run()
    print(cmd1.response)
    if cmd1.response.startswith("0 Record(s)"):
        print("No items selected")
        sys.exit()

    # save the IDs selected above
    cmd2 = uopy.Command("SAVE-LIST " + savedListName)
    cmd2.run()
    print(cmd2.response)
    if "record(s) SAVEd to SELECT list" not in cmd2.response:
        print("No items saved to &SAVEDLISTS&")
        sys.exit()


    with uopy.File("&SAVEDLISTS&") as savedListsFile:
        try:
            savedList = savedListsFile.read(savedListName)
        except uopy.UOError as e:
            print("Error reading &SAVEDLISTS& " + savedListName)
            sys.exit()
        return savedList


status, session = make_connection("..user..", "..pswd..")
print ("STATUS: " + str(status))
print("SESSION: " + str(session))

if status != 0:
    sys.exit()

itemslist = select_items()
if itemslist.count == 0:
    sys.exit()

logFile = uopy.File("EXPORT_LOG")

with uopy.File("CUSTOMER") as custFile:
    for custid in itemslist:
        print("ID: " + str(custid))
        field_arr = uopy.DynArray([ "@ID", "FNAME", "LNAME", "CITY", "ZIP", "PRODID", "SER_NUM", "BUY_DATE", "PAID_DATE", "PRICE" ] )
        custFields = custFile.read_named_fields(custid, field_arr)
        #print(custFields)

        attrNbr = 0
        for attr in custFields:
            attrNbr += 1
            print(str(attrNbr) + " " + str(attr))

        logFileId = "CUSTOMER_" + str(custid) + "_" + str(time.time())
        custStr = '\t'.join(map(str, custFields[3][0])) 
        logFile.write(logFileId, custStr)


logFile.close()
session.close()
